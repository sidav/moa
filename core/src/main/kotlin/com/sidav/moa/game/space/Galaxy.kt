...
class Galaxy(val width: Int, val height: Int) {
    ...
    fun addColony(star: Star, owner: Empire) {
        star.colony = Colony(
            owner, star, 1f, 0f
        )
        owner.onColonizeStar(star)
    }
    ...
    fun canFleetColonizeStar(star: Star, fleet: Fleet): Boolean {
        if (star.isColonized()) return false
        if (star.planetType == PlanetType.NOT_INHABITABLE) return false
        for ((des, count) in fleet.ships) {
            if (count > 0) {
                val allowed = des.getWorstColonizablePlanetType()
                if (allowed != null && star.planetType.betterOrEqualTo(allowed))
                    return true
            }
        }
        return false
    ...
    fun colonizeStarWithFleet(star: Star, fleet: Fleet) {
        if (!canFleetColonizeStar(star, fleet)) error("Can't colonize star, no check was performed?")
        addColony(star, fleet.owner)
        fleet.spendColonyShip(star.planetType)
    }
}
