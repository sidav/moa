...
class Fleet(
    ...
    fun spendColonyShip(planetType: PlanetType) {
        for ((design, _) in ships) {
            val worstVariant = design.getWorstColonizablePlanetType() ?: continue
            if (planetType.betterOrEqualTo(worstVariant)) {
                ships.decreaseOrRemove(design, 1)
                return
            }
        }
