...
class FleetInfoPanel(
    private val galaxy: Galaxy,
    private val onFleetSent: () -> Unit,
    private val onColonize: () -> Unit
) : Table() {
    ...
    fun show(fleet: Fleet) {
        ...
        colonizeButton.addListener(object : ClickListener() {
            override fun clicked(event: InputEvent?, x: Float, y: Float) {
                val currFleet = currentFleet ?: return
                val orbStar = currFleet.orbitingStar ?: return
                galaxy.colonizeStarWithFleet(orbStar, currFleet)
                hide()
                onColonize()
            }
        })
    ...
    private fun updateColonizeButtonState() {
        val currFleet = currentFleet ?: return
        val orbStar = currFleet.orbitingStar ?: return
        colonizeButton.isDisabled = !galaxy.canFleetColonizeStar(orbStar, currFleet)
    }
}
